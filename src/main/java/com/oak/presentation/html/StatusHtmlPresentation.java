...
public class StatusHtmlPresentation extends HtmlPresentation{
    ...
    public Handler handleSuccess = ctx -> {
        File input = new File("src/main/resources/htmls/200.html");
        Document doc = Jsoup.parse(input, "UTF-8");
        ctx.html(doc.html());
    };

    public Handler handleNotFound = ctx -> {
        File input = new File("src/main/resources/htmls/404.html");
        Document doc = Jsoup.parse(input, "UTF-8");
        ctx.html(doc.html());
    };

    public Handler handleForbidden = ctx -> {
        File input = new File("src/main/resources/htmls/403.html");
        Document doc = Jsoup.parse(input, "UTF-8");
        ctx.html(doc.html());
    };
